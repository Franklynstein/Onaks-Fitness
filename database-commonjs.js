      ...
      const defaultPlans = [
        {
          ...
          download_url: 'https://api.onaksfitness.com/uploads/pdfs/generic-pdf.pdf',
...
async function updatePaymentPlan(planId, updateData) {
  try {
    const fields = [];
    const values = [];
...
async function incrementDownloadCount(sessionId) {
  try {
    await db.run(
